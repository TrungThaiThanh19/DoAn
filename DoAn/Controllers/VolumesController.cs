...
namespace DoAn.Controllers
{
	public class VolumesController : Controller
	{
		...
		[HttpPost]
		public async Task<IActionResult> Update(Guid id, string giaTri, string donVi, int trangThai)
		{
			var theTich = await _context.TheTichs.FindAsync(id);
			if (theTich == null) return NotFound();
			...
			else if (donVi.Any(char.IsDigit))
				ModelState.AddModelError("DonVi", "Đơn vị không được chứa số");
		...
		[HttpPost]
		public async Task<IActionResult> Create(string giaTri, string donVi)
		{
			ModelState.Clear();
			decimal giaTriParse = 0;

			string maTheTich = await GenerateMaTheTich();
		...
		[HttpPost]
		public async Task<IActionResult> CreateNew(string giaTri, string donVi)
		{
			ModelState.Clear();
			decimal giaTriParse = 0;
		...
		private async Task<string> GenerateMaTheTich()
		{
			// Lấy số lượng thể tích hiện có, tăng lên 1 để lấy mã mới
			int count = await _context.TheTichs.CountAsync();
			int newNumber = count + 1;
			// Định dạng 2 chữ số, luôn có số 0 phía trước nếu <10
			return $"TT-{newNumber:00}";
		}
	}
}